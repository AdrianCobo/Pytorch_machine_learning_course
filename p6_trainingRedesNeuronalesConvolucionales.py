...
class Net(nn.Module):
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs) # run the init from the parent class (nn.Module)
        self.conv1 = nn.Conv2d(1,32,5) # input is 1 image, 32 output channels, 5x5 kernel/window
        self.conv2 = nn.Conv2d(32,64,5) # input is 32, bc = the first layer output 32. Then we say output will be 64 channels, 5x5 conv
        ...
        self.fc2 = nn.Linear(512,2) # 512 in, 2 out bc we'are doing classes (dog vs cat)

    def convs(self,x):
        # max poooling over 2x2
        x = F.max_pool2d(F.relu(self.conv1(x)), (2,2)) # window of 2x2 for pooling
        x = F.max_pool2d(F.relu(self.conv2(x)), (2,2))
        x = F.max_pool2d(F.relu(self.conv3(x)), (2,2))

        if self._to_linear is None:
            self._to_linear = x[0].shape[0]*x[0].shape[1]*x[0].shape[2] #calculate the dimension of the tensor
            # print(self._to_linear) # -> 512
        return x
    
    def forward(self, x):
        x = self.convs(x)
        x = x.view(-1, self._to_linear) # .view is reshape ... this flatens X before
        x = F.relu(self.fc1(x))
...
val_size = int(len(X)*VAL_PCT) # converting it to int because we are goint to slice our data in groups of it so it needs to be a valid index
print(val_size)
...
for epoch in range(EPOCHS):
    for i in tqdm(range(0, len(train_X), BATCH_SIZE)): # from 0, to the len of x, stepping BACH_SIZE at a time.
